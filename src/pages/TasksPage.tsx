...
import { TASK_PAGE_TEXT } from "../constants/uiConstants";
...
export const TasksPage: React.FC = () => {
  ...
  const isInitialLoading = loading && tasks.length === 0 && totalCount === 0;
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans">
      ...
      <header className="border-b border-slate-800 bg-slate-950/40 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            {loading && !user ? (
              <>
                <Skeleton shape="circle" size="2.5rem" className="shrink-0" />
                <div className="flex flex-col gap-1.5">
                  <Skeleton width="7.5rem" height="1.1rem" />
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 m-0">
                    {user?.email || "Premium Workspace Dashboard"}
                  </p>
                </div>
              </>
            )}
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            {isInitialLoading ? (
              <div className="flex items-center gap-4 sm:gap-6">
                <div className="text-right hidden sm:block flex flex-col items-end gap-1.5">
                  <Skeleton
                    width="3.5rem"
                    height="0.65rem"
                    className="ml-auto"
                ...
                <div className="w-32 bg-slate-800/80 h-2 rounded-full overflow-hidden flex items-center">
        ...
        <section className="relative overflow-hidden bg-gradient-to-r from-indigo-950 to-slate-900 border border-indigo-900/40 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div>
            <h2 className="text-2xl md:text-3xl font-extrabold text-white mb-2 tracking-tight">
              {TASK_PAGE_TEXT.workspaceTitle}
            </h2>
            <p className="text-slate-300 max-w-xl text-sm md:text-base leading-relaxed">
              {TASK_PAGE_TEXT.workspaceDescription}
            </p>
          </div>
          <div className="flex gap-4 w-full sm:w-auto">
            {isInitialLoading ? (
              <>
                <div className="bg-slate-850 border border-slate-700/50 p-4 rounded-xl flex-1 sm:flex-initial text-center sm:text-left min-w-[100px] flex flex-col justify-center gap-2">
                  <Skeleton
                    width="2.5rem"
                    height="1.75rem"
      ...
      <footer className="border-t border-slate-800 bg-slate-950/20 py-6 text-center text-xs text-slate-500 mt-auto">
        <p>&copy; {new Date().getFullYear()} TaskFlow Workspace by AJAY.</p>
      </footer>
    </div>
  );
};
